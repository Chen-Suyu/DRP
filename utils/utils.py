...
def get_others_by_sim_ranking(cur_text, others, embedder, n=4):
    if isinstance(embedder, SentenceTransformer):
        ...
        cur_text_embedding = embedder.encode(cur_text, batch_size=8)
        others_text_embedding = embedder.encode(others_text, batch_size=8)
        similarities = embedder.similarity(cur_text_embedding, others_text_embedding)
...
def get_others_by_kmeans(cur_text, others, embedder, n=4, method="closest"):
    ...
    whole_text_embedding = embedder.encode(whole_text, batch_size=8)
    ...
    available_clusters = [
        c for c in cluster_dict if c != target_label and cluster_dict[c]
    ]
    for cluster in available_clusters:
        cluster_vectors = cluster_dict[cluster]
        if method == "closest":
            distances = [
                np.linalg.norm(vec[1] - cluster_centers[cluster])
...
def get_selected_profile(
    ...
    # compare review_title+description
    # x["title"] is review_title indeed
    profile = profile[-num_documents:]
    ...
    with FileLock(lock_path):
        ...
        for row in reader:
            while len(row) < len(headers):
                row.append("")
        method_index = (
            methods.index(method) + 1 if method in methods else len(reader) - 1
        )
