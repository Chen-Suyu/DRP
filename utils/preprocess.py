...
class GeneralDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        gene_creator, summ_creator = self.prompt_generator(
            user_id, item_title, description, rating, review_title, profile
        )
        return gene_creator, summ_creator, text
...
def create_prompt_generator(
    num_documents, num_retrieved, num_users, asin_reviewers_map
):

    def prompt(user_id, item_title, description, rating, review_title, profile):
        return create_other_inputs(
            num_documents,
            num_retrieved,
            num_users,
            user_id,
...
def create_other_inputs(
    ...
    diff_profile = [
        prof
        for prof in profile
        if len(sorted(asin_reviewers_map[prof["asin"]] - {user_id})) >= num_users
    ]
    selected_profile = utils.get_selected_profile(
        profile, review_title, description, num_documents, num_retrieved
    )
    selected_diff_profile = utils.get_selected_profile(
        diff_profile, review_title, description, num_documents, num_retrieved
    )
    ...
    return create_gene_inp, create_summ_inp


def create_diff_inputs(
    num_documents,
    num_retrieved,
    num_users,
    user_id,
    ...
    def create_prompt(item_title, description, cur, others):
        ...
        for reviewer in reviewers:
            ...
            for pp in other_reviewer_profile:
                if pp["asin"] == p_asin:
                    ...
                    break
        others = utils.get_others_by_kmeans(p_text, others, embedder, n=num_users)
        diff_inp = create_prompt(p_item_title, p_desc, cur, others)
